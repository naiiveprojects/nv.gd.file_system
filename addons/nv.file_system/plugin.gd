...
func _enter_tree() -> void:
	...
	add_tool_menu_item(TITLE_TOOL_MENU_ITEM, switch_file_system_dock)
...
func _exit_tree() -> void:
	...
	var item_view: Button = file_item.get_child(0).get_child(2)
	
	# adjustment
	split_button.visible = true
	split_button.button_pressed = false
	item_view.button_pressed = false
...
func switch_file_system_dock() -> void:
	...
	if !docked:
		...
		await get_tree().process_frame
		file_system_button.button_pressed = docked
	...
	split_button.button_pressed = docked
	item_view.button_pressed = docked
